...
function openEditor(action,id){
 ...
 if(action.includes("user")){
  const u=d.users.find(x=>x.id===id)||{name:"",email:"",role:"User"};
  html=`<form id="editor-form" class="grid"><div class="field"><label>Nama</label><input name="name" required value="${esc(u.name)}"></div><div class="field"><label>Email</label><input name="email" type="email" required value="${esc(u.email)}"></div><div class="field"><label>Role</label><select name="role"><option ${u.role==="User"?"selected":""}>User</option><option ${u.role==="Administrator"?"selected":""}>Administrator</option></select></div><button class="btn primary">Simpan</button></form>`;
