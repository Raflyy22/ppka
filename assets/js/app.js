...
function openSchedule(item={}){
  ...
  $("#schedule-form").onsubmit=async e=>{
    ...
    }catch(err){ toast(err.message+" — mode demo lokal digunakan","error"); if(item.id) state.data.schedules=state.data.schedules.map(x=>x.id===item.id?payload:x); else state.data.schedules.push(payload); closeModal(); render(); }
  };
}
...
function initSimulator(){
  ...
  if(!sc||!route)return;
  simEngine=new PPKAEngine({route,scenario:sc,onUpdate:updateSimulatorUI,onEvent:addSimLog,onFinish:summary=>{
    toast(`Perjalanan selesai — skor ${summary.score}`,"success");
  }});
...
async function boot(){
  try{
    state.data=await loadJSON("/data/db.json");
